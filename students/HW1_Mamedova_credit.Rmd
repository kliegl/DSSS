...
#  geom_errorbar(aes(ymax=dur_M + dur_SE, ymin=dur_M - dur_SE), width=0.15) +
   geom_errorbar(aes(ymax=dur_M + 2*dur_SE, ymin=dur_M - 2*dur_SE), width=0.15,
                 position=position_dodge(width=.1)) +
  scale_colour_manual("", values=c("blue", "red")) +
  scale_y_continuous("Duration [months]") +
  theme_bw(base_size=14) + theme(legend.position=c(0.99, .01), legend.justification = c(0.99, .01))
```
...
tbl_crdt |> 
  ...
  geom_errorbar(aes(ymax=crdt_M + 2*crdt_SE, ymin=crdt_M - 2*crdt_SE), width=0.15,
                position=position_dodge(width=.1)) +
  scale_colour_manual("", values=c("blue", "red")) +
#  scale_y_continuous(("Amount of credit [EUR]") )+
