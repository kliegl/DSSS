---
title: "Contrasts: hypr package"
author: "Reinhold Kliegl"
date: "2022-03-017 (updated: `r Sys.time()`)"
output:
  html_document:
    number_sections: yes
    toc: yes
...
## Analysis of Star run: Need for reciprocal transformation
...
Contrasts are needed for factors.  

## Factor Sex 

### Treatment coding

If you don't specify a contrast, R will specify a treatment contrast for you. This is default.
...
contrasts(dat_run$Sex) <- MASS::contr.sdif(2)  # pcontr.sdif() "lives" in package MASS
...
The (Intercept) estimate looks very different. It is the GrandMean when cohort has the value 0, that is around the first Christmas in history. This is not very plausible, or course. 
...
That's why it is good idea to center covariates at a meaningful value.
...
# recaling does not change the goodness of fit
anova(m3_run, m3b_run)
```

+ (Intercept):  GM in year 2015
...
```{r}
contrasts(dat_run$Cohort) <- contr.treatment(9)
...
It is interesting to compare m3_run and m4_run_trt/sqd
...
The most common contrasts (contr.sum, contr.sdif, contr.treatment) have been coded. If you want to set up your own comparisons, see:

+ Schad et al. (2020)
+ Rabe et al. (2020)

Both papers are in literature.
